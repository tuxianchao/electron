...
const { syncMethods, asyncMethods, promisifiedMethods } = require('@electron/internal/common/web-view-methods')
...
const handleMessageWrapper = function (shouldAwait, channel, handler) {
  const handle = shouldAwait ? 'handleAsync' : 'handle'
...
const handleMessage = handleMessageWrapper.bind(null, false)
const handleMessageAsync = handleMessageWrapper.bind(null, true)
...
handleMessageAsync('ELECTRON_GUEST_VIEW_MANAGER_CALL_PROMISIFIED', async (event, guestInstanceId, method, args) => {
  const guest = getGuestForWebContents(guestInstanceId, event.sender)
  if (!promisifiedMethods.has(method)) {
    throw new Error(`Invalid method: ${method}`)
  }
