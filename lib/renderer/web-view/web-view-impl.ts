...
import { syncMethods, asyncMethods, promisifiedMethods } from '@electron/internal/common/web-view-methods'
...
export const setupMethods = (WebViewElement: typeof ElectronInternal.WebViewElement) => {
  ...
  const createPromisifiedHandler = function (method: string) {
    return function (this: ElectronInternal.WebViewElement, ...args: Array<any>) {
      return ipcRendererUtils.invoke('ELECTRON_GUEST_VIEW_MANAGER_CALL_PROMISIFIED', this.getWebContentsId(), method, args)
    }
  }

  for (const method of promisifiedMethods) {
    (WebViewElement.prototype as Record<string, any>)[method] = createPromisifiedHandler(method)
  }
}
