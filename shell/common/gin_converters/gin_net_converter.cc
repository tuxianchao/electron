...
#include "shell/common/gin_converters/gin_array_converter.h"
#include "shell/common/gin_converters/gin_gurl_converter.h"
// #include "shell/common/gin_converters/gin_string16_converter.h"
...
v8::Local<v8::Value> Converter<scoped_refptr<net::X509Certificate>>::ToV8(
  ...
  return ConvertToV8(isolate, dict);  // TODO(deermichel): correct?
}
  ...
  if (val.request_body) {
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
      ...
      arr->Set(isolate->GetCurrentContext(), static_cast<uint32_t>(i),
               ConvertToV8(isolate, dict))
          .Check();
    }
    dict.Set("uploadData", arr);
  }
  return gin::ConvertToV8(isolate, dict);  // TODO(deermichel): correct?
}
...
//   for (net::HttpRequestHeaders::Iterator
//   it(request->extra_request_headers());
//        it.GetNext();) {
...
//       reader->AsFileReader(); auto file_path =
//       file_reader->path().AsUTF8Unsafe(); upload_data_dict->SetKey("file",
//       base::Value(file_path));
//     } else {
